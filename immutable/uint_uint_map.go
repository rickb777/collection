...
func (mm *UintUintMap) Values() []uint {
	if mm == nil {
		return nil
	}
...
func (mm *UintUintMap) slice() []UintUintTuple {
	if mm == nil {
		return nil
	}

	s := make([]UintUintTuple, 0, len(mm.m))
	for k, v := range mm.m {
...
func (mm *UintUintMap) ToSlice() []UintUintTuple {
	return mm.slice()
}
