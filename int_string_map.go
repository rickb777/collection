...
// options: Comparable:true Stringer:true KeyList:IntList ValueList:StringList Mutable:always
...
func (mm IntStringMap) slice() IntStringTuples {
	s := make(IntStringTuples, 0, len(mm))
	for k, v := range mm {
...
func (mm IntStringMap) mkString3Bytes(before, between, after string) *strings.Builder {
	b := &strings.Builder{}
	b.WriteString(before)
	sep := ""

	keys := make(IntList, 0, len(mm))
	for k, _ := range mm {
		keys = append(keys, k)
	}
	keys.Sorted()
...
// MkString3 concatenates the map key/values as a string, using the prefix, separator and suffix supplied.
func (ts IntStringTuples) MkString3(before, between, after string) string {
	if ts == nil {
		return ""
	}
...
func (ts IntStringTuples) mkString3Bytes(before, between, after string) *strings.Builder {
	b := &strings.Builder{}
	b.WriteString(before)
	sep := ""
	for _, t := range ts {
