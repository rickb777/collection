...
// options: Comparable:true Stringer:true KeyList:collection.Uint64List ValueList:collection.StringList Mutable:always
...
func (mm *Uint64StringMap) mkString3Bytes(before, between, after string) *strings.Builder {
	b := &strings.Builder{}
...
// MkString3 concatenates the map key/values as a string, using the prefix, separator and suffix supplied.
func (ts Uint64StringTuples) MkString3(before, between, after string) string {
	if ts == nil {
		return ""
	}
...
func (ts Uint64StringTuples) mkString3Bytes(before, between, after string) *strings.Builder {
	b := &strings.Builder{}
	b.WriteString(before)
	sep := ""
	for _, t := range ts {
