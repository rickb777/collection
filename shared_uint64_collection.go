...
// SharedUint64Sizer defines an interface for sizing methods on uint64 collections.
type SharedUint64Sizer interface {
	// IsEmpty tests whether SharedUint64Collection is empty.
	IsEmpty() bool

	// NonEmpty tests whether SharedUint64Collection is empty.
	NonEmpty() bool
...
// SharedUint64MkStringer defines an interface for stringer methods on uint64 collections.
type SharedUint64MkStringer interface {
...
// SharedUint64Collection defines an interface for common collection methods on uint64.
type SharedUint64Collection interface {
	SharedUint64Sizer
	SharedUint64MkStringer
	...
	// Exists verifies that one or more elements of SharedUint64Collection return true for the predicate p.
	Exists(p func(uint64) bool) bool

	// Forall verifies that all elements of SharedUint64Collection return true for the predicate p.
	Forall(p func(uint64) bool) bool

	// Foreach iterates over SharedUint64Collection and executes the function f against each element.
	...
	// CountBy gives the number elements of SharedUint64Collection that return true for the predicate p.
	...
	// MinBy returns an element of SharedUint64Collection containing the minimum value, when compared to other elements
	// using a passed func defining ‘less’. In the case of multiple items being equally minimal, the first such
	// element is returned. Panics if there are no elements.
	MinBy(less func(uint64, uint64) bool) uint64

	// MaxBy returns an element of SharedUint64Collection containing the maximum value, when compared to other elements
